...
export class CodexCommandCoordinator {
  ...
  private buildSessionProjectContextLines(
    ...
    title: string,
    languageOrLabels: UiLanguage | { session: string; project: string } = "zh"
  ): string[] {
    const language = typeof languageOrLabels === "string" ? languageOrLabels : "zh";
    const labels = typeof languageOrLabels === "string"
      ? (() => {
          const copy = codexCommandCopy(languageOrLabels);
          return {
            session: copy.currentSession,
            project: copy.currentProject
          };
        })()
      : languageOrLabels;
    return [
